...
public class GifDecoder {  //NOSONAR
    ...
    private ArrayList frames = new ArrayList(); // frames read from current file
     ...
     *
     * @param pName String containing source
     * @return read status code (0 = no errors)
     */
    public int read(String pName) {
        status = STATUS_OK;
        try {
//            String  name = pName.trim().toLowerCase();
            if ((pName.indexOf("file:") >= 0) || (pName.indexOf(":/") > 0)) {
                URL url = new URL(pName);
                in = new BufferedInputStream(url.openStream());
            } else {
                in = new BufferedInputStream(new FileInputStream(pName));
            }
            status = read(in);
        } catch (IOException e) {
            status = STATUS_OPEN_ERROR;
