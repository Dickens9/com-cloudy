...
//					System.out.println(lineNum+":"+line);
            ...
            throw new IllegalArgumentException("Read file as string array error:"+getStackTrace(e));
        }finally{
            close(br);
            close(ir);
            close(fis);
    ...
    /**
     * 扩展数组到指定长度，如果被扩展数组长度大于扩展后的数组，则结原数组数据会选取length长度数据构建新数组
     *
     * @param strAry    被扩展数组
     * @param length    目标长度
     * @return          扩展后的数组
     */
    public static String[] extendArray(String[] strAry, int length){
        String[] retAry = new String[length];
